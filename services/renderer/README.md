...
- Presenter and presenter-slide scenes may bind one absolute local video through `presenterVideos`. The executor re-hashes the clip, probes its dimensions and duration, maps source time into the bounded `activeDurationTicks` interval, and composites it into fixed responsive full, picture-in-picture, or split regions before delivery. The clean scene composition remains after a shorter lip-sync interval ends; presenter frames are never stretched, frozen, or repeated to fill an authored visual tail. Remote/relative paths and arbitrary filter coordinates are not accepted.
