...
test("captures the clean first launch and redesigned global surfaces", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Continue", exact: true }).click();
  await expect(page.getByRole("heading", { name: /choose your model toolkit/i })).toBeVisible();
  await expect(page.getByText(/selecting a downloadable pack starts its verified installation immediately/i)).toBeVisible();
  await page.locator(".aly-onboarding-dialog__content").evaluate((element) => element.scrollTo({ top: 0 }));
  await page.screenshot({ path: `${output}/01-model-toolkit-download-options.png` });
  await page.getByRole("button", { name: /continue without this/i }).click();
