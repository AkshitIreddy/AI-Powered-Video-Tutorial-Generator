...
export type NativeMediaMutationRunner = <T>(operation: () => Promise<T>) => Promise<T>;
export type NativeMediaErrorFormatter = (error: unknown) => string;
...
export async function importNativeEditorMedia(
  ...
  runSerialized: NativeMediaMutationRunner,
  formatError: NativeMediaErrorFormatter,
  ...
  for (const file of files) {
    ...
    try {
      ...
      const receipt = await runSerialized(async () => {
        // Read the head inside the same queue used by project/editor saves. A
        // debounced timeline save may otherwise advance the head between this
        // read and the CAS import, turning valid media into a revision conflict.
        const durable = await projectSnapshotGet(identity);
        const imported = await projectAssetImport({
          ...identity,
        ...
        onHead({ nativeHeadRevisionId: imported.headRevisionId, nativeRevisionNumber: imported.revisionNumber });
        return imported;
      });
      ...
      const errorMessage = formatted || "Media import failed.";
