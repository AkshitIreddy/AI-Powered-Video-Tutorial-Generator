...
describe("native editor media import", () => {
  it("imports files sequentially so every CAS promotion uses the latest head", async () => {
    const invoke = vi.fn(async (request) => ({
      ...
      provenance: { id: `prov-${request.filename}`, origin: "userImport", exportEligible: true, blockers: [] },
    }));
    const result = await importEditorMediaNative(
      ...
      invoke,
      { status: "owned", commercialUse: "allowed", redistribution: "allowed", modelInput: "allowed" },
    );
    ...
    expect(result.assets[0]).toMatchObject({ status: "ready", hash: "a".repeat(64), metadata: { exportEligible: true } });
  });
});
