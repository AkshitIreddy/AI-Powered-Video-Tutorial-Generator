  ...
  return withFrame(props, (
    <g id="body" data-semantic-role="code" data-tutorial-mode={content.kind === "live-code" ? "live-code" : undefined}>
      ...
      {lensRect ? <g data-code-lens="execution">
        ...
        {["CALL", "SPLIT", "RETURN"].map((label, index) => { const y = lensRect.y + lensRect.height * (0.72 + index * 0.09); return <g key={label}><circle cx={lensRect.x + props.scene.metrics.gutter + props.scene.metrics.unit * 0.5} cy={y - 5} r={props.scene.metrics.unit * (index === 1 ? 0.58 : 0.38)} fill={index === 1 ? theme.accent : "#B9C0D4"} /><text x={lensRect.x + props.scene.metrics.gutter + props.scene.metrics.unit * 1.7} y={y} fill={index === 1 ? theme.surface : "#E3E6F1"} fontFamily={theme.fontMono} fontSize={legible(props.scene.metrics.smallSize)} fontWeight={index === 1 ? "820" : "680"}>{label}{index === 0 ? `  ${truncate(functionName, 14)}` : ""}</text></g>; })}
      </g> : null}
    </g>
  ));
}
