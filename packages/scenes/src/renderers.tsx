...
export function WorkedExampleRenderer(props: SceneRendererProps<WorkedExampleContent>) {
  ...
  const body = bodyRect(props);
  const content = props.scene.spec.content;
  const problem: Rect = { x: body.x, y: body.y, width: body.width, height: body.height * 0.22 };
  const answer: Rect = { x: body.x, y: body.y + body.height * 0.78, width: body.width, height: body.height * 0.22 };
  return withFrame(props, <g id="body"><Card rect={problem} theme={theme} tone="warning"><Pill x={problem.x + props.scene.metrics.gutter} y={problem.y + props.scene.metrics.gutter} label="PROBLEM" tone="warning" theme={theme} /><WrappedText text={content.problem} rect={{ x: problem.x + problem.width * 0.22, y: problem.y + props.scene.metrics.gutter * 0.5, width: problem.width * 0.74, height: problem.height - props.scene.metrics.gutter }} theme={theme} fontSize={props.scene.metrics.bodySize} fontWeight="650" maxLines={3} /></Card><BulletList items={content.steps} rect={{ x: body.x + props.scene.metrics.gutter, y: body.y + body.height * 0.27, width: body.width - props.scene.metrics.gutter * 2, height: body.height * 0.46 }} scene={props.scene} frame={props.frame} theme={theme} ordered /><Card rect={answer} theme={theme} tone="secondary"><Pill x={answer.x + props.scene.metrics.gutter} y={answer.y + props.scene.metrics.gutter} label="ANSWER" tone="secondary" theme={theme} /><WrappedText text={content.answer} rect={{ x: answer.x + answer.width * 0.22, y: answer.y + props.scene.metrics.gutter * 0.5, width: answer.width * 0.74, height: answer.height - props.scene.metrics.gutter }} theme={theme} fontSize={props.scene.metrics.bodySize} fontWeight="750" maxLines={3} /></Card></g>);
}
